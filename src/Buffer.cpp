...
bool Buffer::configure(void) {
	...
	this->data_ = this->uniform_vector(n_classes,this->init_val);
...
void Buffer:: setinitval(float init_val){
...
Eigen::VectorXf Buffer::apply(const Eigen::VectorXf& input) {
	...
	// ROS_INFO("BB %d, %f %f",maxIndex,this->data_[0],this->data_[1]);

	return this->data_;
}

bool Buffer::reset(void) {
	this->data_ = this->uniform_vector(this->n_classes,this->init_val);
	return true;
}
